...
public class ListEventFrame extends javax.swing.JFrame {
    ...
    public ListEventFrame() throws DAOException, DNIException {
        initComponents();
        this.model = new EventTableModel(new EventDAO());
        getData();
        this.tabla.setModel(model);
        this.Details.setEdit(false);
        this.Details.setEvent(null);
        this.Details.setComboComplex(
                new SportComplexComboModel(new SportComplexDAO()));
        this.Details.setComboArea(new AreaComboModel(new AreaDAO()));
        this.Details.setComboEquip(
                new EquipmentComboModel(new EquipmentDAO()));
        this.Details.setComboCommissioner(
                new CommissionerComboModel(new CommissionerDAO()));
    ...
    private Event getSelectedEvent() throws DAOException, DNIException {
        int id = (int) tabla.getValueAt(tabla.getSelectedRow(), 0);
        return new EventDAO().getEvent(id);
    }
    ...
    private void BDeleteActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BDeleteActionPerformed
        if (JOptionPane.showConfirmDialog(rootPane, "¿Seguro que quieres borrar "
            ...
            try {
                Event event = getSelectedEvent();
                new EventDAO(event).deleteEvent();
                getData();
            } catch (DAOException ex) {
                ex.getCause();
            } catch (DNIException ex) {
    ...
    private void BSaveActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BSaveActionPerformed
        try {
            if (Details.checkData()) {
                ...
                if (event.getId() == null) {
                    new EventDAO(event).insertEvent();
                } else {
                    new EventDAO(event).modifyEvent();
                }
                Details.setEvent(null);
